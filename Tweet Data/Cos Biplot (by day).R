...
day.data = aggregate(text ~ time + handle, data=raw.data,FUN=paste, collapse=' ')

row.sample = sample(1:nrow(day.data),size=n,replace=FALSE)
tweets = as.data.frame(cbind(1:n,day.data[row.sample,colnames(day.data)%in%c("text")]))
colnames(tweets) = c("doc_id","text")
user = as.data.frame(day.data[row.sample,colnames(day.data)%in%c("handle")])
names(user) = "user"
...
tfidf = DocumentTermMatrix(corpus,control=list(weighing=weightTfIdf))
...
mdsplot <- ggplot() + labs(title=NULL,x="",y="") +
  ...
  geom_text(data=Z.mds.Cos.df,aes(x=Z1,y=Z2,label=observations,color=user),size=4) + 
  geom_point(data=LD.axis,aes(x=V1,y=V2),color="black") +
  geom_text(data=LD.axis,aes(x=V1,y=V2,label=attributes,fontface="bold"),color="black",cex=5) + 
...
# png(file = "~/Dropbox/Student Seminar/Pictures/Trump_Clinton.png", width = 1024, height = 768, units = 'px')
print(mdsplot)
dev.off()
