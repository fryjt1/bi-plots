...
#Z.mds = Euclidean.MDS(X,nstart=50)
#Z.mds = Rotation(Z.mds)

Z.mds = Euclidean.MDS.SA(X)

stress.j = rep(NA,p)
for(j in 1:p)
{
  X.temp = X[,-j] 
  stress.j[j] = mean( (Euclidean.Dist(X.temp) - Euclidean.Dist(as.matrix(Z.mds)))^2 )
}
...
for(i in 1:p)
{
  ...
  for(j in 1:length(plot.seq))
  {
    ...
    for(k in 1:nrow(starts))
    {
      a = rep(0,p)
      a[i] = plot.seq[j]
      opt = optim(par=starts[k,],fn=Euclidean.Stress,method="L-BFGS-B",
                  lower=rep(-4,2),upper=rep(4,2),X=X,Z=Z.mds,a=a)
      sols[k,] = opt$par
      mins[k] = opt$value
    }
...
mdsplot <- ggplot() + labs(title="MDS (Euclidean) Projection",x="",y="") +
