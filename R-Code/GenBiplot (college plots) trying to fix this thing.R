...
# dev.off()
print(mdsplot)
stress.all = sum((as.matrix(dist(X,"manhattan"))-as.matrix(dist(Z.mds)))^2)
# 
# head(LD.axis.df)
# 
# par(mfrow=c(3,3))
# for(i in 1:p)
# {
#   plot(LD.axis.df[which(LD.axis.df$attribute==attributes.all[i]),1:2],type="l",
#        main=attributes.all[i])
# }
# 
# cbind(attributes.all,rowMeans(fit$stress.mat))
